            ...
            */
            ...
            foreach($conceptos as $con){
                DB::connection('DB_Serverr')
                    ->insert('insert into conceptos (clave_concepto,concepto,formula,naturaleza,manejo,cantidad,importe,monto,isr,imss,infonavit,estatal,isr_uma,isr_porcentaje,imss_uma,imss_porcentaje,seleccionado,created_at,updated_at)
                                values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                                [$con['clave_concepto'],$con['concepto'],$con['formula'],$con['tipo'],$con['manejo'],$con['cantidad'],$con['importe'],$con['monto'],$con['isr'],$con['imss'],$con['infonavit'],$con['estatal'],$con['isr_uma'],$con['isr_porcentaje'], $con['imss_uma'],$con['imss_porcentaje'],$con['seleccionado'],$fecha_periodo,$fecha_periodo]);
            }
            ...
            $primavacacional = Collect([
                ...
                foreach($primavacacional as $prima){
                    DB::connection('DB_Serverr')
                        ->insert('insert into prestaciones (anio,dias,prima_vacacional,aguinaldo,created_at,updated_at)
                                    values (?,?,?,?,?,?)',
            ...
            DB::connection('DB_Serverr')
                ->insert('insert into periodos(numero,fecha_inicio,fecha_fin,fecha_pago,diasPeriodo,status_periodo,created_at,updated_at)
                            VALUES(?,?,?,?,?,?,?,?)'
                            ,[$numero,$fechaInicio,$fechaFin,$fechaPago,$datos->tipoPeriodo,1,$fecha_periodo,$fecha_periodo]);
